...
function createImage(url) {
  return external_jQuery_default().Deferred(function (deferred) {
    ...
    $img.one('load', function () {
      ...
      display: 'none'
    }).appendTo(document.body).attr('src', $.sanitizeUrl(url));
  }).promise();
}
  ...
  function Editor(context) {
    ...
    this.createLink = this.wrapCommand(function (linkInfo) {
      ...
      if (typeof linkUrl === 'string') {
        linkUrl = $.sanitizeUrl(linkUrl).trim();
      }

      if (_this.options.onCreateLink) {
        linkUrl = _this.options.onCreateLink($.sanitizeUrl(linkUrl));
      } else if (checkProtocol) {
        // if url doesn't have any protocol and not even a relative or a label, use http:// as default
        linkUrl = /^([A-Za-z][A-Za-z0-9+-.]*\:|#|\/)/.test($.sanitizeUrl(linkUrl)) ? linkUrl : _this.options.defaultProtocol + $.sanitizeUrl(linkUrl); 
        
        if (linkUrl === '//') {
          alert("Url was not valid and removed");
          remove(_this.options.onCreateLink($.sanitizeUrl(linkUrl)));
      ...
      if (isTextChanged) {
        rng = rng.deleteContents();
        var anchor = rng.insertNode(external_jQuery_default()('<A>' + $.sanitizeUrl(linkText) + '</A>')[0]);
        anchors.push(anchor);
      } else {
...
var CodeView = /*#__PURE__*/function () {
  ...
  Codeview_createClass(CodeView, [{
    ...
    value: function deactivate() {
      ...
      var value = this.purify($.sanitize(dom.value(this.$codable, this.options.prettifyHtml)) || dom.emptyPara);
